...
const codeExamples = {
  javascript: {
    authentication: {
      description:
        "Authenticate with CoinDuka API using your client credentials to receive an access token.",
      code: `// Using fetch
const response = await fetch('https://api.coinduka.com/v1/auth', {
  ...
  body: JSON.stringify({
    ...
    sendTransaction: {
      description:
        "Send money to another user using their phone number or email. The recipient will receive an SMS/email notification.",
...
const response = await fetch('https://api.coinduka.com/v1/transactions', {
  ...
  body: JSON.stringify({
    ...
    queryTransaction: {
      description:
        "Query the status and details of a specific transaction using its ID.",
...
const response = await axios.post<AuthResponse>('https://api.coinduka.com/v1/auth', {
  ...
  headers: {
    ...
    sendTransaction: {
      description:
        "Send transactions with type safety and better error handling.",
      code: `// Send a transaction
...
const response = await axios.get<TransactionDetails>(
  ...
  python: {
    authentication: {
      description:
        "Simple authentication using the requests library in Python.",
      code: `# Using requests
import requests
...
const ApiDocs = () => {
  ...
  const handleRegenerateKey = () => {
    toast({
      ...
      description:
        "Your new API key has been generated and copied to clipboard.",
    });
  };
  ...
  const handleTryRequest = async (endpoint: string) => {
    try {
      ...
      setApiResponse({
        code: randomCode,
        data: mockResponses[randomCode as keyof typeof mockResponses],
      });
    } catch (error) {
      setApiResponse({
        code: 500,
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-800 via-fuchsia-500 to-pink-400 dark:from-purple-900 dark:via-gray-900 dark:to-black p-4 md:p-8">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <Card className="bg-[#221F26]/80 border-[#0EA5E9]/20 backdrop-blur-lg">
          ...
          <CardContent>
            <p className="text-white/70 text-sm">The API is rate limited to:</p>
