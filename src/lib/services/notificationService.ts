...
export const NotificationService = {
  ...
  notifyTransactionStatus: (
    ...
    const statusType = status === "CONFIRMED" ? "success" : 
                      status === "CANCELLED" ? "error" : "info";
    
    NotificationService.addNotification({
      title: `Transaction ${status.toLowerCase()}`,
      message: `Your ${type} transaction (ID: ${txId.substring(0, 8)}...) has been ${status.toLowerCase()}.`,
  ...
  notifyKycStatus: (status: string): void => {
    const statusType = status === "APPROVED" ? "success" : 
