...
const BalanceCard = () => {
  ...
  const calculateTotalBalance = () => {
    if (selectedCrypto === "ALL") {
      return cryptoCurrencies.reduce((total, crypto) => total + (crypto.price || 0), 0);
    }
  ...
  return (
    <GlassCard className="relative animate-scale-in">
      ...
      <div className={`text-3xl font-bold mb-2 ${!showBalance ? 'blur-content' : ''}`}>
        {formatPrice(calculateTotalBalance())}
      </div>
      
      <div className={`text-sm text-gray-600 ${!showBalance ? 'blur-content' : ''}`}>
